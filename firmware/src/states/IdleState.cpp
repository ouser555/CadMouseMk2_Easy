...
void IdleState::runMotionPipeline(float dt, unsigned long now) {
  ...
  if (!sensorController.readRaw(raw)) {
    // Sensor read failure: keep the host from extrapolating the last
    // live motion by flushing zero axes. Button state is still valid
    // (different hardware path) so we preserve it.
    hidController.sendNeutral(inputController.buttonBits());
    return;
  }
#else
